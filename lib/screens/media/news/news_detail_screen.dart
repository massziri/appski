            ...
            actions: [
              TextButton(
                ...
                child: Container(
                  ...
                  child: Text(lang == 'en' ? 'МК' : 'EN', style: const TextStyle(color: AppColors.darkNavy, fontWeight: FontWeight.bold, fontSize: 12)),
                ),
              ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(children: [
                    ...
                    Text(article.author, style: const TextStyle(color: AppColors.grey, fontSize: 12)),
                    const SizedBox(width: 16),
                    const Icon(Icons.calendar_today, size: 14, color: AppColors.grey),
                    const SizedBox(width: 4),
                    Text(DateFormat('MMM d, yyyy').format(article.publishedAt), style: const TextStyle(color: AppColors.grey, fontSize: 12)),
                    const SizedBox(width: 16),
                    const Icon(Icons.timer, size: 14, color: AppColors.grey),
                    const SizedBox(width: 4),
                    Text('${article.readingTime} min read', style: const TextStyle(color: AppColors.grey, fontSize: 12)),
                  ]),
                  const SizedBox(height: 20),
                  const Divider(color: AppColors.darkCard),
                  const SizedBox(height: 16),
                  Text(article.getBody(lang), style: const TextStyle(color: AppColors.lightGrey, fontSize: 15, height: 1.7)),
                  ...
                  ...MockData.news.where((n) => n.newsId != newsId).take(3).map((related) => Container(
                    ...
                    child: Text(related.getTitle(lang), style: const TextStyle(color: AppColors.white, fontSize: 13)),
                  )),
                ],
              ),
            ),
